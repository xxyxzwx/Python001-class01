...
class QipaoshuiSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for pinglun in comments:
            ...
            #评论开头含有表情列表第一项为空，需转化
            comment_content = pinglun.xpath('./div[@class="comment_conWrap"]/div[1]/p/span/text()').extract()
            content = ''.join(comment_content).replace(' ','')
            print(productType,productName,username,content,comment_time)
            item['productType'] = productType
            item['productName'] = productName
            item['username'] = username
            item['comment_content'] = content
            item['sentiments'] = dataAnalysis(content)
